...
def count_sets(cards):
    ...
    """
    dupes = []  # contains list of cards already processed to avoid duplicates
    if len(cards) != 12:  # not the right number of cards
    ...
    for card in cards:
        if card in dupes:
            ...
            dupes.append(card)
        if len(card) != 4:  # if a card is not formatted correctly
            raise ValueError("Cards must have exactly four digits")
        for i in range(4):
            goodchars = ['0', '1', '2']  # list of acceptable card values
            if card[i] not in goodchars:
                raise ValueError("Cards can only contain '0', '1', or '2'")
    total = []
    for i in range(len(cards) - 2):  # calculates number of matches
        for j in range(1 + 1, len(cards) - 1):
...
def is_set(a, b, c):
    ...
    """
    if a == b or a == c or b == c:  # fail if duplicates exist
        return False
    for i in range(len(a)):
        if (int(a[i]) + int(b[i]) + int(c[i])) % 3 != 0:  # fail if cardvalues don't create a set
            return False
    return True
